  ...
  methods: {
    ...
    initPageInfo() {
      if (this.$frontmatter.article == undefined || this.$frontmatter.article) {
        ...
        const { eachFileWords, pageView, pageIteration, readingTime } = this.$themeConfig.blogInfo;
     ...
     * 挂载目标到页面上
     */
    mountedView(template, mountedIntervalTime = 100, moutedParentEvent = ".articleInfo-wrap > .articleInfo > .info") {
      let i = 0;
